#!/usr/bin/env python3
"""test_tile_audit_truth.py — locks tests/tile_audit_truth.csv as the source
of truth for 0.2 patch 1500-1620 tile identifications.

The user-curated CSV (`tests/tile_audit_truth.csv`, regenerable from the
PNG audit) is canonical: every entry MUST be reflected in
`_MENU_GLYPHS` (or `_SPECIAL_BIGRAMS` for tile 1500's
...
CSV_PATH = Path(__file__).resolve().parent / "tile_audit_truth.csv"
...
class TestTileAuditTruth(unittest.TestCase):
    def setUp(self):
        if not CSV_PATH.exists():
            self.skipTest(f"CSV not present at {CSV_PATH}")
        from font_tools import _MENU_GLYPHS
        self.overrides = _MENU_GLYPHS
        self.csv = parse_csv()
    ...
    def test_special_tiles_not_in_menu_glyphs(self):
        """1488/1489/1490 (name keys) are drawn by OUR
        _NAME_ENTRY_BUTTON_TILES art (2026-06-11: arrows + condensed END),
        not by the 0.2 patch override table."""
        for tile in (1488, 1489, 1490):
            self.assertNotIn(
                tile, self.overrides,
                f"tile {tile} is owned by _NAME_ENTRY_BUTTON_TILES",
            )
